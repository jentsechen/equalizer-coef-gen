...
def design_complex_fir_remez(
    ...
    for it in range(n_iter):
        ...
        if verbose:
            msg = (f"  iter {it+1:3d}: "
                   f"pb_peak={pb_peak:.6f}  sb_peak={sb_peak:.6f}")
            if K_tb > 0:
...
def design_eqz_lowpass_remez(
    ...
    print(f"  In-band RMS   |H − Hd| : {pb_err_rm:.5f}  ({20*np.log10(pb_err_rm +1e-12):.1f} dB)")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--f-pass',   type=float, default=162.5, # 162.5
                   help='Passband half-bandwidth in MHz')
    p.add_argument('--f-stop',   type=float, default=250, # 250
    ...
    p.add_argument('--verbose',  action='store_true',
                   help='Print per-iteration Lawson diagnostics')
    return p
