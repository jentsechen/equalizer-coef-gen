...
class TxEqualizerDesign:
    ...
    def __matched_filter(self, rx_waveform):
        return abs(
            np.convolve(rx_waveform, self.desired_signal.conjugate(), mode="valid")
        )
    ...
    def gen_eqz_out(self, meas_data, n_taps, sim_en=False):
        ...
        eqz_out = np.convolve(self.desired_signal, coef.conjugate(), mode="full")[
            0 : len(self.desired_signal)
        ]
    ...
    def gen_mf_out(self, data):
        ...
        assert width % 2 == 0
        mf_out_up_smp = resample_poly(
            mf_out[(center - int(width / 2)) : (center + int(width / 2))],
            up=self.matched_filter_up_factor,
            down=1,
        )
    ...
    def find_pslr_db(self, mf_out):
        ...
        for i in range(1, len(mf_out - 1)):
            if (
                (i != main_lobe_peak_index)
                and (mf_out[i] > mf_out[i - 1])
                and (mf_out[i] >= mf_out[i + 1])
            ):
...
class TxEqzDesByChirp(TxEqualizerDesign):
    def __init__(self, fs_is_750mhz: bool = False):
        ...
        qntz_format = qt(
            sign=eSign.Signed, int_bit=0, frac_bit=15, msb=eMSB.Sat, lsb=eLSB.Rnd
        )
    ...
    def gen_coef(
        self,
        file_path="./meas_sig/result/ch4_bfr_eqz.npy",
        ...
        if print_en:
            print(
                self.__print_arr(np.array(np.round(coef.real * 2**14), dtype=int), "re")
            )
            print(
                self.__print_arr(np.array(np.round(coef.imag * 2**14), dtype=int), "im")
            )
        return coef
    ...
    def load_legacy_coef(self, ch_idx):
        if ch_idx == 4:
            re = (
                np.array([-2028, 14481, -200, 3284, -4386, 3863, -3318, 1655, -244])
                / 2**14
            )
            im = np.array([-575, -167, 2078, -1669, -965, 1060, -806, 766, 278]) / 2**14
        elif ch_idx == 5:
            re = (
                np.array([6, 15143, -10132, 8647, -6985, 5327, -3362, 1239, -247])
                / 2**14
            )
...
class TxEqzDesByOfdm(TxEqualizerDesign):
    def __init__(self):
        ...
        desired_signal = (
            desired_signal_j["re"] + 1j * np.array(desired_signal_j["im"])
        )[0:4096]
        qntz_format = qt(
